...
decreeSchema.pre("validate", function (next) {
  if (this.parte_vetada && !this.ley_vetada) {
    next(
      new Error(
        `No se puede especificar la parte vetada sin una ley vetada. Ley N° ${this.num}/${this.anho}`
      )
    );
  } else {
    next();
...
const db = mongoose.connection.useDb("decretos", { useCache: true });
export default db.model("Decree", decreeSchema);
